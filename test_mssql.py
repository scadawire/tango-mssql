...
from MsSQL import MsSQL as Driver
DRIVER_NAME = "MsSQL"
...
class MockCursor:
    """
    Emulates the dict-style cursor the drivers assume (result['field']).
    Understands the driver's own SELECT/UPDATE templates well enough to give a
    real read-after-write round-trip, keyed by column name; arbitrary queries
    (the `sql` command) just return the preconfigured rowcount / fetchall.
    """
    def __init__(self):
    ...
    def execute(self, sql, params=None):
        ...
        if head.startswith("SELECT") and " AS FIELD" in head:
            ...
            self._last = {"field": self.store[col]} if col in self.store else None
